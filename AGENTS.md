...
go run ./cmd/vmctl ssh      # SSH into guest as user "dev" @ 192.168.64.10
```
...
internal/vmctl/            # all Go code (cobra.go, vm.go, config.go, util.go, build_vfkit.go, bootstrap_script.go, web*.go, sync*.go, tunnel*.go)
web/static/                # vanilla HTML/CSS/JS frontend (served by echo/v5)
scripts/                   # guest-bootstrap.sh, e2e-test.sh
docs/                      # specs.md + superpowers design docs
```

- CLI framework: `spf13/cobra` (see `cobra.go:9` for subcommand registration)
- Web server: `labstack/echo/v5` on `VM_MANAGER_PORT` (default 8080), serves `web/static/` + REST API
- Config: env vars overridden by optional `.vmctl.env` in repo root (`config.go`)
...
- Guest bootstrap: `bootstrap_script.go` generates a shell script from Go template; `scripts/guest-bootstrap.sh` is the standalone version
- Sync: `sync_config.go` (JSON config `.vmctl.sync`), `sync_copy.go` (rsync), `sync_git.go` (git push/pull)
- Tunnels: `tunnel_config.go` (JSON config `.vmctl.tunnels`), `tunnel_manager.go` (SSH -L/-R via PID files)

## Conventions & Gotchas

- `.vmctl.env` must use **absolute paths** — no `~` or `$HOME` expansion
- `.vmctl.*` config files (`.vmctl.env`, `.vmctl.sync`, `.vmctl.tunnels`) are gitignored
- Runtime state lives under `.vm/void-dev/` (gitignored): `disk.img`, `vmlinuz`, `initramfs.img`, `bootstrap.done`, `vfkit.log`, `serial.log`, `vfkit.pid`, `efi-vars.fd`
- Base images in `images/` (gitignored)
- Default VM: 6 CPUs, 6144 MiB RAM, 100G disk, fixed IP 192.168.64.10
- Default user: `dev` / password: `dev`; root: `root` / password: `root`
